...
slope_degrees = 0.25 # simulates road bumps
slope = slope_degrees*np.pi/180
track_length = 100 # in meters
Speed_limit_kph = 25 # Defining top speed per regulations
...
ax6.plot(ratios, final_time_hist, color="black", linewidth=1.3)
for i in range (len(ratios)) :
    ax5.scatter(ratios[i], final_velo_hist[i], marker = "o", color=colors[i], alpha = 1, zorder=3, s=72)
    ax6.scatter(ratios[i], final_time_hist[i], marker = "o", color=colors[i], alpha = 1, zorder=3, s=72)
...
ax2.axhline(Speed_limit_kph, color="red", linestyle="--", linewidth=1, label=f"{Speed_limit_kph} km/h regulation limit")
...
ax5.set_xlabel("Reduction ratio G")
ax5.set_ylabel("Top speed (km/h)")
ax5.grid(True)
ax5.legend(fontsize=8)

if best_ratio is not None :
    ax6.scatter([best_ratio], [final_time_hist[best_idx]], marker="s", s=200, facecolors="none", edgecolors="red", linewidths=2, zorder=6, label=f"Lowest ratio meeting {Speed_limit_kph} km/h speed limit")
    ax6.legend(fontsize=8)
ax6.set_xlabel("Reduction ratio G")
...
fig.savefig(my_path + "/drivetrain_sweep_results.png", dpi=150)
plt.show()
