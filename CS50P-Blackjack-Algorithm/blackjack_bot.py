...
def bot_decision(): 
    ...
    if len(player_distributed_cards_this_round) == 2 and player_distributed_cards_this_round[0].split(" of ")[0] == player_distributed_cards_this_round[1].split(" of ")[0] :
        with open(r"C:\VS Code projects\Personal\CS50P - Blackjack\perfect_blackjack_split.csv", "r", encoding="utf-8") as file:
        ...
        if Ace_in_hand(player_distributed_cards_this_round, dealer_distributed_cards_this_round)["Ace in player cards"] == False : 
            with open (r"C:\VS Code projects\Personal\CS50P - Blackjack\perfect_blackjack_hard.csv", "r", encoding="utf-8") as file :
                reader = csv.DictReader(file)
                rows = list(reader)
                for row in rows : 
            ...
            with open(r"C:\VS Code projects\Personal\CS50P - Blackjack\perfect_blackjack_soft.csv", "r", encoding="utf-8") as file:
                reader = csv.DictReader(file)
                rows = list(reader)
                for row in rows : 
                    if player_score == int(row["player score"].strip()) : 
...
def simulation_performance(): 
    global number_wins, number_losses, number_pushes, total_tokens, tokens_tracking
    performance_dictionary = {
        ...
        "average return per hand" : (np.mean(tokens_tracking)-10000)/len(tokens_tracking),
