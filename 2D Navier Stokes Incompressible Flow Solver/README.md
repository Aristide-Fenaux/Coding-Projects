...
$$\begin{cases} \text{div}(\vec{u}) = 0 \\ \dfrac{\partial \vec{u}}{\partial t} + \text{grad}(\vec{u})\,\vec{u} = -\dfrac{1}{\rho}\text{grad}(p) + \nu\,\text{Lap}(\vec{u}) \end{cases}$$
...
![Flow field animation around the wing](Results/simulation_wing_preview.gif)

*(downsampled preview — [full-resolution video](Results/simulation_wing_final.mp4))*

And around a cylinder, for comparison:

![Flow field animation around the cylinder](Results/simulation_cylinder_preview.gif)

*(downsampled preview — [full-resolution video](Results/simulation_cylinder_final.mp4))*
...
![Lift and drag coefficient full history](Results/lift_and_drag_coefficients_final.png)

![Lift and drag coefficient croped history](Results/lift_and_drag_coefficients.png)
...
- Results shown here are for a single Reynolds number, however the solver could be swept across Reynolds numbers and angle of attack to build up a proper $C_L$/$C_D$ dataset for different conditions. 
